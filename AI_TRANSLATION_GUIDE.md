...
生产门（每一项都需要用户新的明确批准，不得自行执行）：

1. 对生产应用迁移 `0013`（`npm run build` 会执行 migration，注意环境）。
2. 生产去重 `--apply`：需先补全 105 个冲突组的决策文件。
3. 迁移 `0014` 唯一索引：仅当生产重复组归零后。
4. 真实 Worker canary/长跑。

`9330` 重复组 / `105` 冲突组是 2026-07-24 快照。开发/测试只能使用临时
`file:` 数据库，不得让开发命令读取生产 Turso 环境。
